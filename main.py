...
class LinghuiStudioPlugin(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self.dashboard_api = LinghuiDashboardApi(self)
    ...
    def _get_rebellious_hint(self, message: str, uid: str, event=None) -> str:
        ...
        if self._is_in_obedient_whitelist(uid, event):
            logger.debug(f"LinghuiStudio: 用户 {uid} 在顺从白名单中（或为管理员），返回顺从提示")
    ...
    def _load_persona_scenes(self):
        ...
        logger.debug(f"LinghuiStudio: 已加载 {len(self._persona_scene_map)} 个场景提示词")
    ...
    async def initialize(self):
        ...
        if os.path.exists(config_path):
            try:
                ...
                if removed_from_backup > 0:
                    ...
                    logger.info(
                        f"LinghuiStudio: 已从 dynamic_config.json 清理 {removed_from_backup} 个废弃强力模式字段")
                ...
                logger.info(
                    f"LinghuiStudio: dynamic_config.json 恢复完成，恢复 {restored_count} 项，跳过 {skipped_count} 项"
                )
                if skip_reasons:
                    logger.info(f"LinghuiStudio: dynamic_config.json 跳过原因统计: {skip_reasons}")
        ...
        logger.info(
            f"LinghuiStudio 已加载 v3.5.0 | LLM智能判断: {auto_detect_status} | 上下文轮数: {self._context_rounds}")
        ...
        try:
            ...
            try:
                if hasattr(self.conf, "save_config") and callable(self.conf.save_config):
                    ...
                    saved = True
                elif hasattr(self.conf, "save") and callable(self.conf.save):
                    self.conf.save()
                    saved = True
                ...
                logger.warning(f"LinghuiStudio: AstrBot 原生配置保存失败，将继续写入动态备份: {e}")
            ...
            # Existing command handlers call this synchronous helper. Refresh
            # the cached route clients in the background so a model/channel
            # switch takes effect for the next request.
            try:
                asyncio.get_running_loop().create_task(self.api_mgr.refresh())
            except RuntimeError:
                pass

        except Exception as e:
            logger.error(f"LinghuiStudio Config Save Failed: {e}")
    ...
    def _get_custom_drawing_negative_prompt(self) -> str:
        """Return the optional cross-channel negative clause for #bnn/#画."""
        return str(self.conf.get("custom_drawing_negative_prompt", "") or "").strip()
    ...
    def _get_quota_str(self, deduction: dict, uid: str, gid: Optional[str] = None) -> str:
        ...
        if deduction["source"] == "free":
            reason = str(deduction.get("exemption_reason", "") or "额度豁免")
            return f"不限次数（{reason}） | 用户: {user_count} | 群组: {group_count}"
    ...
    async def _resolve_contextual_image_inputs(
        ...
        if context_images:
            logger.info(f"LinghuiStudio: 使用最近图片上下文 {len(context_images)} 张作为本次图生图输入")
        ...
        if recent_generated:
            logger.info(f"LinghuiStudio: 使用最近生成缓存 {len(recent_generated)} 张作为本次图生图输入")
        ...
        if images:
            ...
            logger.info(f"LinghuiStudio: 从上下文图片记录恢复 {len(images)} 张作为本次图生图输入")
    ...
    async def _check_quota(self, event, uid, gid, cost) -> dict:
        ...
        decision = self.access_policy.evaluate(uid, gid, is_admin=self.is_admin(event))
        if not decision.allowed:
            return {"allowed": False, "source": None, "msg": decision.message, "access_level": decision.level}

        # Access and quota exemption are intentionally separate. A group in
        # group_whitelist gains permission to draw, but still consumes its
        # configured balance unless it is explicitly in unlimited_groups.
        if decision.unlimited:
            return {
                "allowed": True,
                "source": "free",
                "msg": "",
                "access_level": decision.level,
                "exemption_reason": decision.message,
            }

        # Check credit limits only after access has been granted.
        ...
        if not enable_u and not enable_g:
            return {"allowed": True, "source": "free", "msg": "", "access_level": decision.level}

        u_bal = self.data_mgr.get_user_count(uid)
        if enable_u and u_bal >= cost:
            return {"allowed": True, "source": "user", "msg": "", "access_level": decision.level}
        if gid and enable_g:
            g_bal = self.data_mgr.get_group_count(gid)
            if g_bal >= cost:
                return {"allowed": True, "source": "group", "msg": "", "access_level": decision.level}
        else:
            g_bal = 0
        ...
        return {
            "allowed": False,
            "source": None,
            "msg": f"额度不足（需要 {cost} 次，个人剩余 {u_bal}，群剩余 {g_bal}），请联系管理员补充。",
            "access_level": decision.level,
        }
    ...
    @filter.llm_tool(name="linghui_check_user_status")
    async def check_user_status_tool(self, event: AstrMessageEvent):
        '''检查用户是否是高权限VIP用户。

        【调用优化指南（重要！）】：
        1. 如果用户当前请求是【生成图片、处理图片、批量处理图片】，你【绝对不需要】调用此工具！因为在你调用对应的图片工具（如 linghui_draw_image）时，系统会自动在返回结果中告诉你用户的 VIP 状态。为了避免让用户久等，请直接调用图片工具，跳过此工具！
        ...
        if self._is_vip_user(uid, event):
            ...
            logger.debug(f"LinghuiStudio: 用户 {uid} 被识别为 {user_type}")
            return self._build_master_identity_hint(uid, event, explicit_master=not is_admin)
        else:
            # 普通用户，返回正常提示
            logger.debug(f"LinghuiStudio: 用户 {uid} 被识别为普通用户")
            return "【用户身份】普通用户。请按照你的正常人设和性格来回复；如果启用了叛逆模式，可以自然地带一点傲娇、调侃或小脾气，但不要像流程化应答。"

    @filter.llm_tool(name="linghui_draw_image")
    async def text_to_image_tool(self, event: AstrMessageEvent, prompt: str, count: int = 1,
                                 aspect_ratio: str = "auto", resolution: str = "auto"):
        '''根据文本描述生成图片（文生图）。

        【排他性条件（极度重要！）】
        - 如果用户是向你要**你自己的 / Bot本人 / 人设角色**的照片、自拍、写真、露脸、外貌、穿搭、换装、cos、动作展示、与上图合影或同框，【严禁】调用此工具！必须使用 linghui_persona_photo 工具！
        - 如果用户说“修改上面那张 / 改刚才那张 / 继续改 / 在这个基础上改”等依赖上一张图片的请求，【严禁】调用此工具！必须使用 linghui_edit_image，用户不需要重新引用图片。
        ...
        if self._persona_mode and self._looks_like_persona_photo_request(persona_request_text):
            logger.info("linghui_draw_image 命中人设请求兜底分流，改用 linghui_persona_photo")
    ...
    async def image_edit_tool(self, event: AstrMessageEvent, prompt: str, use_message_images: bool = True,
        ...
        persona_request_text = " ".join([str(getattr(event, "message_str", "") or ""), str(prompt or "")])
        if self._persona_mode and self._looks_like_persona_photo_request(persona_request_text):
            logger.info("linghui_edit_image 命中人设请求兜底分流，改用 linghui_persona_photo")
            inferred_count = self._infer_requested_count_from_text(persona_request_text, default=1, multi_default=3)
            return await self.persona_photo_tool(
                event=event,
                scene_hint="",
    ...
    async def on_figurine_request(self, event: AstrMessageEvent, ctx=None):
        ...
        if self._is_message_processed(dedup_key):
            logger.debug(f"LinghuiStudio: 消息 {msg_id} 已被处理，跳过重复执行")
            return
    ...
    async def on_avatar_image_to_image(self, event: AstrMessageEvent, ctx=None):
        ...
        gid = norm_id(event.get_group_id())
        if not uid.isdigit():
            yield event.chain_result([Plain("这个指令需要 QQ 数字账号，当前平台无法读取 QQ 头像。")])
            return
        ...
        avatar = await self.img_mgr.get_avatar(uid)
        if not avatar:
            yield event.chain_result([Plain("QQ 头像获取失败，请稍后重试或直接发送参考图。")])
            return

        if not prompt:
            prompt = "Create a polished portrait based on the reference avatar, preserving recognizable facial features and overall identity."
        final_prompt, preset_name, extra_rules = self._process_prompt_and_preset(prompt)
    ...
    async def on_checkin(self, event: AstrMessageEvent, ctx=None):
        ...
        decision = self.access_policy.evaluate(uid, gid, is_admin=self.is_admin(event))
        if not decision.allowed:
            yield event.chain_result([Plain(decision.message)])
            return
    ...
    async def on_message_record(self, event: AstrMessageEvent, ctx=None):
        ...
        try:
            ...
            logger.debug(f"LinghuiStudio: 消息记录失败: {e}")

        # 不阻断事件传递
        return

    @filter.llm_tool(name="linghui_smart_generate")
    async def smart_generate_tool(self, event: AstrMessageEvent, user_request: str = ""):
        ...
        4. 如果用户是在索要你自己的照片、自拍、写真、长相，请不要用这个工具，必须改用 linghui_persona_photo
        ...
        logger.info(f"LinghuiStudio 智能判断: {task_type} (置信度: {confidence:.2f}) - {reason}")
    ...
    @filter.llm_tool(name="linghui_pack_images_to_pdf")
    async def pack_images_to_pdf_tool(self, event: AstrMessageEvent, max_images: int = 10):
        ...
        2. 如果用户要求"把这些图片手办化/处理后再打包"，请使用 `linghui_batch_process` 工具并设置 output_as_pdf=True。
    ...
    @filter.llm_tool(name="linghui_batch_process")
    ...
    @filter.llm_tool(name="linghui_batch_process_concurrent")
    ...
    @filter.llm_tool(name="linghui_persona_photo")
    async def persona_photo_tool(self, event: AstrMessageEvent, scene_hint: str = "", extra_request: str = "",
        ...
        无论要求多少张，都【必须且只能】使用此工具，绝对不能使用 linghui_draw_image。
        ...
        - 用户只说“某角色cos某角色给我看”“画一个cos某角色的人”等第三方人物创作，应该使用 linghui_draw_image
    ...
    async def on_batch_process_cmd(self, event: AstrMessageEvent, ctx=None):
        ...
        if self._is_message_processed(dedup_key):
            logger.debug(f"LinghuiStudio: 批量处理消息 {msg_id} 已被处理，跳过重复执行")
            return
        ...
        # These values were referenced below without being initialized in the
        # upstream handler, causing every batch request to raise NameError.
        uid = norm_id(event.get_sender_id())
        gid = norm_id(event.get_group_id())
        pdf_extracted_urls: List[str] = []
